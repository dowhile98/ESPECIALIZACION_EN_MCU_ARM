...
int main (void)
{
  ...
  /* Fundamental sample time is set to: '0.5' s */
  if (SysTick_Config((uint32_t)(SystemCoreClock/2.0))) {
    autoReloadTimerLoopVal_S = 1;
    do {
      autoReloadTimerLoopVal_S++;
    } while ((uint32_t)(SystemCoreClock/2.0)/autoReloadTimerLoopVal_S >
             SysTick_LOAD_RELOAD_Msk);

    SysTick_Config((uint32_t)(SystemCoreClock/2.0)/autoReloadTimerLoopVal_S);
  }
  ...
  while (1) {
    ...
    if (remainAutoReloadTimerLoopVal_S == 0) {
      ...
      if (OverrunFlags[0]) {
        rtmSetErrorStatus(SimpleGPIO_M, "Overrun");
      }
